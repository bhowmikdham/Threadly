...
def main():
    ...
    if settings.app_env != "prod":
        raise SystemExit(
            "Staging requires APP_ENV=prod; development auth is not permitted."
        )
    ...
    if url.host != "postgres" or url.drivername != "postgresql+asyncpg":
        raise SystemExit(
            "DATABASE_URL must target this stack's postgres service via asyncpg."
        )
    if settings.inference_provider != "bedrock":
        raise SystemExit("This staging deployment requires INFERENCE_PROVIDER=bedrock.")
    if settings.bedrock_region not in SUPPORTED_BEDROCK_REGIONS:
        raise SystemExit(
            "Staging supports Bedrock only in ap-southeast-2 or ap-southeast-4."
        )
    if settings.conversation_enabled:
        ...
        if not re.fullmatch(pattern, settings.bedrock_model_id):
            raise SystemExit(
                "Conversation requires the reviewed Haiku inference-profile ARN "
                "in BEDROCK_REGION."
            )
        if not settings.bedrock_mail_processing_acknowledged:
            raise SystemExit(
                "Conversation requires BEDROCK_MAIL_PROCESSING_ACKNOWLEDGED=true "
                "after the account logging and content boundary review."
            )
    if (
        settings.gmail_source_mode != "on_demand"
    ...
    if settings.email_writes_enabled or settings.calendar_writes_enabled:
        ...
        if (
            ...
            and not settings.calendar_reconciliation_enabled
        ):
            raise SystemExit(
    ...
    if settings.conversation_enabled:
        print(
            "Contextual conversation configuration passed; a live model check is still required."
        )
    elif settings.bedrock_model_id:
        print(
            "Bedrock model configured; access and generation still require a separate smoke test."
        )
    else:
    ...
    if not all(
        ...
        print(
            "PENDING: Google OAuth configuration. Live Gmail/Calendar login is unavailable."
        )
